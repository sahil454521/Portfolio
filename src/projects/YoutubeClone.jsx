...
const YoutubeClone = () => {
  ...
  const codeSnippets = [
    {
      ...
      language: "javascript",
      code: `const fetchVideos = async (searchQuery = '', maxResults = 15) => {
  try {
    ...
    },
    {
      ...
      language: "javascript",
      code: `function InfiniteScroll({ onLoadMore }) {
  ...
  return (
    <div ref={lastElementRef} className="h-10 w-full flex justify-center items-center">
    ...
    {
      title: "Video Recommendation Logic",
      language: "javascript",
      code: `// Get video recommendations based on current video and user history
const getRecommendations = (currentVideoId, userHistory) => {
  // First, fetch related videos for the current video
  return fetchRelatedVideos(currentVideoId)
    .then(relatedVideos => {
      if (!userHistory || userHistory.length === 0) {
        return relatedVideos;
      }
      
      // If we have user history, enhance recommendations
      // Extract tags and categories from user's watched videos
      const userPreferences = analyzeUserHistory(userHistory);
      
      // Sort related videos based on user preferences
      return sortVideosByRelevance(relatedVideos, userPreferences);
    })
    .catch(error => {
      console.error('Failed to get recommendations:', error);
      // Fallback to trending videos if recommendations fail
